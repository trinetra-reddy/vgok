...
import { createForum, updateForum, ForumData } from "@/services/forumService";
...
interface CreateOrEditForumModalProps {
  ...
  forum?: { id: string; title: string; content: string, description?: string }; // If present = Edit mode
...
export const CreateOrEditForumModal = ({
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<{ title: string; content: string; description: string }>();
  ...
  const onSubmit = async (data: ForumData) => {
    try {
      if (user) {
        if (isEdit && forum?.id) {
          await updateForum(forum.id, data, user?.token);
          toast.success("Forum updated successfully.");
        } else {
          await createForum(data, user?.token);
